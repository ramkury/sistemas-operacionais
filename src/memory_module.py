...
def check_mem(priority, base_register, mem_block, alloc):
    ...
    if alloc == 1:
        if priority == 0:
            ...
            if mem_block > 64:
                print("Real-time proccess memory block is too wide from memory range available.")
            else:
                ...
                for i in range(len(rt_mem_space)):
                    if mem_block <= int(rt_mem_space[i][1]) and int(rt_mem_space[i][0]) >= 0 and int(rt_mem_space[i][0]) <= 64 and mem_block + int(rt_mem_space[i][0]) <= 64:
                        alloc_mem(int(rt_mem_space[i][0]), mem_block, i, 0)
                    else: 
                        print("Real-time proccess memory block is too wide from memory range available.")
        else:
            # Verifica se o espaço de memória pedido foi do tamanho correto definido de no máximo 960 blocos
            if mem_block > 960:
                print("User proccess memory block is too wide from memory range.")
            else:
                # Verifica se há espaço suficiente para alocar o processo. Esta verificação se dá pela conta: addr_inicial_da_mem_livre + tamanho_restante_da_mem < espaço_pedido_pelo_processo
                ...
                for i in range(len(usr_mem_space)):
                    if mem_block <= int(usr_mem_space[i][1]) and int(usr_mem_space[i][0]) <= 1024  and mem_block + int(usr_mem_space[i][0]) <= 1024:
                        alloc_mem(int(usr_mem_space[i][0]), mem_block, i, 1)
                    else:
                        print("User proccess memory block is too wide from memory range available.")
    else:
    # Caso o pedido seja de desalocar processo na memória (alloc != 1)
        index = 0
        if priority == 0:
            # Loop para fazer verificação de onde o espaço livre irá entrar na lista de memória livre e se irá ocorres coalescência
            for i in range(len(rt_mem_space)):
                if int(base_register) + int(mem_block) == int(rt_mem_space[i][0]):
                    index = i
                    break
                elif int(base_register) + int(mem_block) < int(rt_mem_space[i][0]):
                    index = i
                    break
                elif int(rt_mem_space[i][0]) < int(base_register):
                    index = i
            
            disalloc_mem(base_register, mem_block, index, priority)
        else:
            # Loop para fazer verificação de onde o espaço livre irá entrar na lista de memória livre e se irá ocorres coalescência
            for i in range(len(rt_mem_space)):
                if int(base_register) + int(mem_block) == int(rt_mem_space[i][0]):
...
# Função faz a desalocação de memória. Retira o processo terminado da memória e libera espaço.
def disalloc_mem(base_register, proccess_size, index, priority):
    ...
    if priority == 0:
        ...
        elif int(rt_mem_space[index][0]) < int(base_register):
            base = int(rt_mem_space[index][0])
            rt_mem_space.pop(index)
            new_size = int(base_register) - int(base)
            aux = (int(base), int(new_size))
            rt_mem_space.insert(index, aux)
            aux = (int(base_register), int(mem_block))
    ...
    # Processos com prioridades 1, 2 ou 3 repetem o passo acima, porém na sua lista própria
        ...
        if len(usr_mem_space) == 0:
            ...
            aux = (int(base_register), int(proccess_size))
            usr_mem_space.insert(index, aux)
        elif int(usr_mem_space[index][0]) < int(base_register):
            base = int(usr_mem_space[index][0])
            usr_mem_space.pop(index)
            new_size = int(base_register) - int(base)
            aux = (int(base), int(new_size))
            usr_mem_space.insert(index, aux)
...
# check_mem(priority, base_register, mem_block, 0)
...
# TODO: linkar com a lista de processo e dispatcher
# TODO: registrador de base do processo para ser informado em qual posição de memória ele foi alocado